...
As part of the project affiliated with [NSF POSE](https://new.nsf.gov/funding/opportunities/pathways-enable-open-source-ecosystems-pose) and based on expanding the open-source ecosystem around `data.table`, several interviews had to be conducted under [NSF's I-CORPS](https://new.nsf.gov/funding/initiatives/i-corps) program (Winter 2024 Cohort) to gather information as to how `data.table` as an open-source project can improve, along with ways to discover the project's sustainability. The timeline for the interviews was originally planned to be from January to March '24 with a required interview count of 100. However, the program downscaled in time to start from the 17th of January to four weeks after, and so did the count, to a total of 60 (just wrapped up my final week as I'm writing this!). [Issue#5880](https://github.com/Rdatatable/data.table/issues/5880) mentions this, and has a link to the Google Doc that contains the list of people interviewed.

I was assigned by Toby to do these interviews and serve as the EL (Entrepreneurial Lead) for the `data.table` team (which aside from this, involved tasks such as making and giving various presentations). Having successfully completed the program and conducted the interviews, it's time to share the insights I gathered from them as a source of open-ended knowledge for the community.

But before you head below to read those parts, I would like to convey a big 'Thank you!' to everyone who took part in this, right from making availability and scheduling to providing comprehensive feedback and all the while being extremely communicative. I sincerely appreciate it. Not just the value brought to the table, but also in being great people to talk with in general!


## Motivations in contributing

People see different values that bring them forward to contribute to the project. Varying from person to person, these include:
...
- Financial gain or incentives from working on the project. None of the people interviewed were paid to do so, but nearly all of them agreed that a paid position could be a win-win for the interested ones (and as an example, the author of `data.table`, Matt, was paid by H20.ai previously).
...
As gathered from the interviewees, there are a couple of reasons that prevent them from contributing to `data.table`. These include:
...
Specific areas were identified by the interviewed population that they would like to see improvement in or be worked upon.
...
- `dataset[get("categoricalColumn")]` could be optimized further (for reference, please check [getDTeval](https://cran.r-project.org/web/packages/getDTeval/vignettes/Introduction_to_getDTeval.html)).
...
- More often a mild inconvenience than a common source of error, but to one, the masking of functions from other packages (such as `between`, `first`, and `last` from `dplyr`, or `transpose` from `purrr`) is something they do not like.
...
Things to potentially do or keep in mind for the agenda going forward:
...
Some positive feedback and in general, statements that interviewed people said about `data.table`:

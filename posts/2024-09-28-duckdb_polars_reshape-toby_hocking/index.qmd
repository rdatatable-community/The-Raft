...
In the plot above, the computation time in seconds is plotted as a function of `N`, the number of input rows to reshape. The horizontal reference line is drawn at `r seconds.limit` seconds, and the `N` highlighted corresponds to the throughput given that time limit. We see that `data.table` is slightly faster than `polars` (within 2x), and both are much faster than `duckdb` (more than 10x).
...
`duckdb` support long-to-wide reshape via [the SQL `PIVOT` command](https://duckdb.org/docs/sql/statements/pivot.html), which can be used to recover the original iris data via the command below:
...
Below the code uses `mean` as an aggregation function.
...
Finally, we can do multiple aggregation via the code below, which requires enumerating each combination of aggregation function and input column to aggregate.
...
Below we conduct an `atime` benchmark to measure the time/memory usage of the reshape operation (without including the copy operation).
...
slightly slower (less than 2x). Memory usage is not informative (top facet), because `atime` does not measure memory usage outside of R.
Below we run a more complex benchmark which also measures time/memory for the copy operation (in addition to the reshape).
...
The result above shows that `data.table` is quite a bit faster than the others (5x). Again 

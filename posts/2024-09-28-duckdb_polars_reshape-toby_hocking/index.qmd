...
In the plot above, the computation time in seconds is plotted as a function of `N`, the number of input rows to reshape. The horizontal reference line is drawn at `r seconds.limit` seconds, and the `N` highlighted corresponds to the throughput given that time limit. We see that `data.table` is comparable to, or slightly faster than `polars` (within 2x), and both are much faster than `duckdb` (more than 10x).
...
terms of computation time. In this comparison, `data.table` is
...
The output above is consistent with the results from `data.table::dcast`, and the original iris data, although the names are unusual. The next reshape example below shows that we need to create a dummy variable to use as the `on` argument.
...
Below we conduct an `atime` benchmark to measure the computation time of the reshape operation (without including the copy operation).
...
The result above shows that `data.table` is quite a bit faster than the others (5x). 

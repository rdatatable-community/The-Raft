...
The National Science Foundation's [POSE](https://www.nsf.gov/pubs/2022/nsf22572/nsf22572.htm) program (Pathways to Enable Open-Source Ecosystems) has given a grant to Toby Dylan Hocking (PI of project) to expand the open-source ecosystem around `data.table`. The title of our project is "POSE: Phase II: Expanding the data.table ecosystem for efficient big data manipulation in R." We proposed a two year project to create more documentation and testing infrastructure, in order to help expand the `data.table` ecosystem. This blog post explains what we proposed to do to improve the testing infrastructure.

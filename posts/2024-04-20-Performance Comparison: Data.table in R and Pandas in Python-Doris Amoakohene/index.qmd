...
The plot shows the asymptotic reference lines, which define the estimated asymptotic time and memory complexity. The output and figure will include violet reference lines, in which the text labels can be interpreted in terms of Big O notation (asymptotic time and memory usage).
...
The graph provides a closer look at larger values of N, indicating that data.table::fread outperforms pandas.read_csv , particularly as N increases. Complexity notations such as log N, N log N, and N^2 are used to describe the asymptotic time/memory usage.
...
Also in this graph we see that data.table::melt performs for complexity notations to pd.melt

...
Non-API entry points used in `data.table`
=========================================
...
* `src/utils.c`: need to copy columns that share memory or are ALTREP
  [preparing zero `TRUELENGTH`s][datatable_copyShared1] (wouldn't the
  `SET_TRUELENGTH` call always fail for `ALTREP` columns?), [marking

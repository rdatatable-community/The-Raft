...
by reference. Since `data.frame`s and `data.table`s are lists, and lists
in R are value types with pass-by-value semantics (see above), adding or
removing a column to one the normally involves allocating a new vector
of pointers to columns (performing a "shallow duplicate").

The [over-allocated vector of column pointers][datatable_overallocation]
(together with the `names` vector) remember their original length in the
`TRUELENGTH` field and can be resized in place by gradually increasing
their `LENGTH`. The walrus operator, `:=`, has since become [the
defining feature of data.table][datatable_logo]. `data.table` version
1.8.8 (March 2013) [saw][news188] the addition of the `fread` function
that [over-allocated and later truncated the individual
columns][datatable_stretch_column] using the same approach.
...
[news188]: https://github.com/Rdatatable/data.table/blob/6a15f8617de121a406cee97b22e83e0c2c4bb034/NEWS.0.md#new-features-5

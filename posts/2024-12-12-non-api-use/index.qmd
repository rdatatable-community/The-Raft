...
* When copying shortened objects without the `GROWABLE_BIT` set, R
  ...
  * For this and other reasons, `data.table`s have a special
    [`.internal.selfrep` attribute][datatable_assign_selfref] attached
...
and `data2`. A simple implementation of a shortened, expandable vector
could hold a full-length vector in the `data1` slot and its
pretend-length as a one-element `REALSXP` value in the `data2` slot.
(Currently, `R_xlen_t` values are limited by the largest integer
precisely representable in an IEEE `double` value, which is $2^{52}$.)
The overallocated class will need to implement the following methods:
...
reallocation will invalidate any cached data pointers, it must be only
used from inside `data.table`, not from the ALTREP methods.

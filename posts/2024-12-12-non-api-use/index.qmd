...
`tools:::nonAPI` grew from `r length(nonAPI.3_3)` to `r
length(nonAPI.4_4)` entries, and the package maintainers had to adapt or
face archival of their packages.
...
objects if possible and requested to. If the <a href="#NAMED">reference
count</a> of its argument is _above_ 1, it will operate upon and return
...
undocumented function <a href="#ATTRIB-all">`SET_ATTRIB`</a>. For
...
for which <a href="#ATTRIB-all">no API exists</a>. The solution is to
...
value types with <a href="#NAMED">pass-by-value semantics</a>, adding or
removing a column to one the normally involves allocating a new list
...
<a href="#TRUELENGTH-mark">uses `TRUELENGTH` to sort strings</a>. R-3.4
...
The encoding conversions take $O(n+k)$ time and space; the `TRUELENGTH`
bookkeeping takes $O(n)$ space and time (thanks to the exponential
`realloc` trick).
...
Since there does't seem to be any intent to allow using R API to place
...
would need $O(n)$ memory, $O(n)$ time to initialise, and average $O(k)$
...
A truly lazy implementation could just use [`std::unordered_map<SEXP,
int>`][cppreference_unordered_map] (at the cost of requiring C++11,
...
* In `src/utils.c`, columns share memory or are ALTREP must be copied.
  ...
  `TRUELENGTH` is set on an object, accessing the object through a
  different pointer and seeting `TRUELENGTH` set will prove that the
  object has been previously visited. The code first [prepares zero
...
The same solution as <a href="#TRUELENGTH-mark">above</a> can be used
here, with the same downsides of having to allocate memory for the hash
table and the potential to have worst-case $O(kn)$ time for $k$ lookups
...
[datatable_nafill_ATTRIB]: https://github.com/Rdatatable/data.table/blob/master/src/nafill.c#L216

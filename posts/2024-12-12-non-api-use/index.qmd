...
wants to evaluate a function call. <!-- TODO: provide other examples of
eval() -->
 ...
 * The current implementation performs extra work to un-ALTREP lists and
   vectors given to `data.table`, precisely because it's impossible to
   `SET_TRUELENGTH` on them. This will also need to be refactored.
...
[in R itself][R_unique_PTRHASH]. A hash table would need $O(n)$ memory
and average $O(k)$ time for lookup. <!-- TODO: how much time for
preparation? compare against a realistic case with UTF-8 conversion and
savetl() with reallocation -->
...
<!-- TODO: any other solutions? different data structure? -->
...
<!-- TODO: can it use non-compact rownames instead? -->
